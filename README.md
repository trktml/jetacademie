...
Curriculum entries are stored in PostgreSQL (`curriculum_entries` table, seeded across all 6 Belgium grades). The curriculum page loads only the selected grade; switching grades fetches that grade through `/api/curriculum?sinif=1..6` and caches it in the browser. The database seed and category freshness checks run once per server process, rather than on every page visit.
